...
data KNormal
    ...
    | If   { cond :: [KNormal], then' :: [KNormal], else' :: [KNormal], pos :: Maybe L.AlexPosn }
    deriving Eq
...
knormalize typeEnv = mapM knormalize'
    ...
    knormalize' = \case
        (C.Fun label args type' terms pos) -> Fun label (map arg2Val args) type' <$> knormalizeTerm type' terms <*> pure (Just pos)
        (C.Bind label type' terms pos)     -> Bind label type'                   <$> knormalizeTerm type' terms <*> pure (Just pos)
        where
    ...
    knormalizeTerm retType terms = 
        ...
        knormalizeTerm' retVar = \case
            ...
            C.If{..}        -> do
                cond' <- E.liftEff #io $ knormalizeTerm (T.Type "Bool") [cond]  
                then' <- E.liftEff #io $ knormalizeTerm (valType retVar) terms
            ...
            C.Call{..}      -> do
                ...
                forM_ (zip3 targs args types) $ \(term, argName, type') -> do
...
instance Show KNormal where
    ...
    show (If c e e' _)      = unlines (map show c) ++ "if ( #return ) {\n" ++ (addIndent . unlines $ map show e) 
                            ++ "} else {\n" ++ (addIndent . unlines $ map show e') ++ "}"
