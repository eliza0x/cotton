...
{-# LANGUAGE OverloadedStrings, OverloadedLabels, LambdaCase, DeriveFunctor, FlexibleContexts #-}
...
data Stmt
    = Fun       { elabel :: Text, eargs  :: [Var], etype :: T.Type, eterms :: [Term], epos :: L.AlexPosn }
...
data Term
    ...
    | SemiColon { term :: Term, term' :: Term, type''' :: Maybe T.Type, pos :: L.AlexPosn } -- 連結
...
inspectImplicitArgs stmts = runInspect $ mapM_ (inspectStmt "global") stmts
    ...
    inspectTerm block = \case
        ...
        (P.SemiColon t t' _ _) -> inspectTerm block t >> inspectTerm block t'
...
closure typeEnv exprs = concatMap (W.execWriter . unnest) exprs
    ...
    unnest = \case
        ...
        (P.Fun l as t exprs p) -> do
            ...
            let args = argsFromName (implicitArgs `implicitArgsBy` l) ++ pargsToArgs as
    ...
    unnest' = \case
        (P.Bind l t exprs p) -> do
            terms <- M.catMaybes <$> mapM unnest' exprs
            let (t:ts) = terms
            return . Just $ TBind l (typeOf l) (foldl (\t t' -> SemiColon t t' Nothing p) t ts) p
        (P.Fun l as t exprs p) -> do
            terms <- M.catMaybes <$> mapM unnest' exprs
            let args = argsFromName (implicitArgs `implicitArgsBy` l) ++ pargsToArgs as
    ...
    unnestTerm = \case
        ...
        (P.SemiColon term term' type' pos) -> SemiColon <$> unnestTerm term <*> unnestTerm term' <*> pure type' <*> pure pos
